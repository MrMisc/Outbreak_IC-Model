...
//Resolution
const STEP:[[usize;3];3] = [[4,4,2],[100,100,10],[2,2,2]];  //Unit distance of segments ->Could be used to make homogeneous zoning (Might not be very flexible a modelling decision)
const HOUR_STEP: f64 = 4.0; //Number of times hosts move per hour
const LENGTH: usize =22; //How long do you want the simulation to be?
//Infection/Colonization module
// ------------Do only colonized hosts spread disease or do infected hosts spread
const HOST_0:usize = 3;
...
//Space
const LISTOFPROBABILITIES:[f64;3] = [0.25,0.25,0.25]; //Probability of transfer of samonella per zone - starting from zone 0 onwards
const GRIDSIZE:[[f64;3];3] = [[80.0,32.0,16.0],[300.0,300.0,10.0],[4000.0,2.0,2.0]];
...
const NO_OF_HOSTS_PER_SEGMENT:[u64;3] = [10,3000,1];
//Space --- Segment ID
const TRANSFERS_ONLY_WITHIN:[bool;3] = [false,true,false]; //Boolean that informs simulation to only allow transmissions to occur WITHIN segments, not between adjacent segments
//Fly option
...
const SLAUGHTER_POINT:usize = 2; //Somewhere in zone {}, the hosts are slaughtered/killed and will cease to produce any eggs or faeces
...
//Transfer parameters
const ages:[f64;3] = [9.0,2.0,1.0]; //Time hosts are expected spend in each region minimally
//Collection
...
fn main(){
    ...
    chickens = host::infect_multiple(chickens,GRIDSIZE[zone_to_infect][0] as u64/2,GRIDSIZE[zone_to_infect][1] as u64/2,GRIDSIZE[zone_to_infect][2] as u64/2,HOST_0,0, true);
